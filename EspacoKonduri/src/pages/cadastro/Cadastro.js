...
import { Platform, Alert, ActivityIndicator, ScrollView, KeyboardAvoidingView } from "react-native";
          ...
          <ChecklistContainer>
            {REQUISITOS_SENHA.map((req) => (
              <ChecklistRow key={req.key}>
                <Ionicons
                  name={
                    checksAtuais[req.key] ? "checkmark-circle" : "ellipse-outline"
                  }
                  size={14}
                  color={checksAtuais[req.key] ? "#2E5A1E" : "#A99B8F"}
                />
                <ChecklistText ok={checksAtuais[req.key]}>
                  {req.label}
                </ChecklistText>
              </ChecklistRow>
            ))}
